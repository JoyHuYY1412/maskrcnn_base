...
def main(argv):
    ...
    cls_weight_new = torch.Tensor(271+160*3, 1024)
    nn.init.normal_(cls_weight_new, std=0.01)
    cls_weight_new[:271+160*2] = model['module.roi_heads.box.predictor.cls_score.weight']
    model['module.roi_heads.box.predictor.cls_score.weight'] = cls_weight_new

    reg_weight_new = torch.Tensor((271+160*3)*4, 1024)
    nn.init.normal_(reg_weight_new, std=0.001)
    reg_weight_new[:(271+160*2)*4] = model['module.roi_heads.box.predictor.bbox_pred.weight']
    model['module.roi_heads.box.predictor.bbox_pred.weight'] = reg_weight_new
    
    reg_bias_new = torch.Tensor((271+160*3)*4)
    nn.init.constant_(reg_bias_new, 0)
    reg_bias_new[:(271+160*2)*4] = model['module.roi_heads.box.predictor.bbox_pred.bias']
